...
class restore_gradebook_structure_step extends restore_structure_step {
     ...
     */
     protected function execute_condition() {
        ...
        $backupversion = $this->get_task()->get_info()->moodle_version; // The version of Moodle at the time of backup.
        ...
        $moodle30fixed = 2015111604.10;
        $moodle31fixed = 2016052300.02;
        $moodle32fixed = 2016052600.01;
        $is30orloweraffected = $backuprelease <= 3.0 && $backupversion < $moodle30fixed;
        $is31affected = $backuprelease == 3.1 && $backupversion < $moodle31fixed;
        $is32affected = $backuprelease == 3.2 && $backupversion < $moodle32fixed;
        if ($is30orloweraffected || $is31affected || $is32affected) {
    ...
    protected function rewrite_step_backup_file_for_legacy_freeze($filepath) {
        ...
        if ($fr && $fw) {
            ...
            if (!feof($fr)) {
                throw new moodle_exception('Error while attempting to rewrite the gradebook step file.');
            }
            fclose($fr);
            fclose($fw);
            if (!rename($newfile, $filepath)) {
                throw new moodle_exception('Error while attempting to rename the gradebook step file.');
            }
        } else {
            if ($fr) {
                fclose($fr);
