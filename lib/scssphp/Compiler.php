...
class Compiler
{
    ...
    protected function compileChild($child, OutputBlock $out)
    {
        ...
        switch ($child[0]) {
            ...
            case Type::T_INCLUDE:
                ...
                if ($isaddsize) {
                    // var_dump($this->env->parent->parent->store);
                }
    ...
    public function get($name, $shouldThrow = true, Environment $env = null)
    {
        ...
        for (;;) {
            // $name == 'i' && var_dump('Start') & var_dump(array_keys($env->store));
            // $name == 'i' && file_put_contents('/tmp/test.txt', serialize($env)) & die();

            if (array_key_exists($normalizedName, $env->store)) {
                // $name == 'i' && var_dump('Found');
                return $env->store[$normalizedName];
            }

            if (! $hasNamespace && isset($env->marker)) {
                // $name == 'i' && var_dump('Marker');
                if (!$nextIsRoot && !empty($env->store['%content'])) {
                    // $name == 'i' && var_dump('Scope');
