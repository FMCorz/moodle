...
M.form_filemanager.init = function(Y, options) {
    ...
    Y.extend(FileManagerHelper, Y.Base, {
        ...
        check_buttons: function() {
            ...
            // Disable the "Download all" button when there is no files/folders in the current view.
            var button_download = Y.one("#btndwn-"+this.client_id);
            if (this.options.list.length == 0) {
                button_download.setStyle('display', 'none');
            } else {
                button_download.setStyle('display', 'inline');
            }
        },
