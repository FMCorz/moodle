...
define([
...
], function($, Chartjs, Axis, ChartBubble, Base) {
    ...
    Output.prototype.getDatasets = function() {
        var sets = this._chart.getSeries().map(function(series) {
            return {
                ...
                data: series.getValues().map(function(value) {
    ...
    Output.prototype._makeConfig = function() {
        ...
        if (config.data.labels && this._chart.getType() === ChartBubble.prototype.TYPE) {
            config.options.scales = config.options.scales || {};
            config.options.scales.xAxes = config.options.scales.xAxes || [];
            config.options.scales.xAxes[0] = config.options.scales.xAxes[0] || {};
            config.options.scales.xAxes[0].ticks = config.options.scales.xAxes[0].ticks || {};
            config.options.scales.xAxes[0].ticks.callback = function(value) {
                return config.data.labels[parseInt(value, 10)] || '';
            };
        }

        if (this._chart.getType() === ChartBubble.prototype.TYPE) {
            config.options.tooltips = config.options.tooltips || {};
            config.options.tooltips.callbacks = {
                label: function(tooltipItem, data) {
                    var s = data.datasets[tooltipItem.datasetIndex].data[tooltipItem.index];
                    if (s.r) {
                        return Math.round(s.r / 30 * 10000) / 100 + '%';
                    }
                    return s.y;
                }
            };
