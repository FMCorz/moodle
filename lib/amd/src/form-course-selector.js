...
define(['core/ajax', 'jquery'], function(ajax, $) {
    ...
    return /** @alias module:core/form-course-selector */ {
        ...
        processResults: function(selector, data) {
            ...
            var results = [], i = 0;
            var excludelist = $(selector).data('exclude').split(',');
            for (i = 0; i < data.courses.length; i++) {
                if (excludelist.indexOf(data.courses[i].id) === -1) {
                    results.push({ value: data.courses[i].id, label: data.courses[i].fullname });
                }
            }
            return results;
        },
