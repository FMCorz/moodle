...
define(['core/mustache',
    ...
    /** @var {Promise[]} templateCache - Cache of loaded (or loading) templates. */
    var templateCache = {};

    /** @type {String} The pix icon template cache. */
    var pixIconCache = "";
    ...
    Renderer.prototype.getTemplate = function(templateName, async) {
        ...
        if (cached) {
            templateCache[searchKey] = $.when(cached);
            return templateCache[searchKey];
        }
        ...
        templateCache[searchKey] = promises[0].then(function(templateSource) {
    ...
    Renderer.prototype.pixHelper = function(context, sectionText, helper) {
        ...
        result = mustache.render(pixIconCache, templatecontext, this.partialHelper.bind(this));
        return result.trim();
    };
    ...
    Renderer.prototype.doRender = function(templateSource, context, themeName) {
        ...
        var loadPixTemplate = this.getTemplate('core/pix_icon', true).then(function(tpl) {
            pixIconCache = tpl;
        });

        return loadPixTemplate.then(function() {
