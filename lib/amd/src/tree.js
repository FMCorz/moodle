...
define(['jquery'], function($) {
    ...
    var SELECTORS = {
        ITEM: '[role=treeitem]',
        GROUP: '[role=group], [role=treeitem][data-requires-ajax=true]',
    ...
    Tree.prototype.isGroupItem = function(item) {
        var owns = this.treeRoot.find('#' + item.attr('aria-owns'));
        return (owns && owns.is(SELECTORS.GROUP)) || item.is(SELECTORS.GROUP);
    };
    ...
    Tree.prototype.getGroupFromItem = function(item) {
        return this.treeRoot.find('#' + item.attr('aria-owns')) || item.children(SELECTORS.GROUP);
    };
