...
function quiz_get_user_attempts($quizids, $userid, $status = 'finished', $includepreviews = false) {
    ...
    $attemptssql = "quiz $insql AND userid = :userid";
    $params += $inparams;
    ...
    return $DB->get_records_select('quiz_attempts',
            $attemptssql . $previewclause . $statuscondition,
            $params, 'attempt ASC');
}
...
function quiz_print_overview($courses, &$htmlarray) {
    ...
    $finishedstates = [quiz_attempt::FINISHED, quiz_attempt::ABANDONED];
    $attemptsinfo = [];
    $quizids = [];
    foreach ($quizzes as $quiz) {
        $quizids[] = $quiz->id;
        $attemptsinfo[$quiz->id] = ['count' => 0, 'last' => null];
    }
    $attempts = quiz_get_user_attempts($quizids, $USER->id, 'all');
    foreach ($attempts as $attempt) {
        $attemptsinfo[$quiz->id]['count']++;
        $attemptsinfo[$quiz->id]['last'] = $attempt;
    }
    unset($attempts);
    ...
    foreach ($quizzes as $quiz) {
        if ($quiz->timeclose >= $now && $quiz->timeopen < $now) {
            ...
            if (has_capability('mod/quiz:viewreports', $context)) {
                ...
                if (isset($USER->id)) {
                    if (in_array($attemptsinfo[$quiz->id]['last']->state, $finishedstates)) {
