...
class provider implements
        ...
        $sql = "
            SELECT fc.id
              ...
              JOIN {course_modules} cm
               ...
               AND cm.module = m.id
             WHERE cm.id = :cmid";
        $params = ['cmid' => $context->instanceid, 'feedback' => 'feedback'];
        $DB->delete_records_select('feedback_valuetmp', sprintf("completed IN ($sql)", 'feedback_completedtmp'), $params);
        $DB->delete_records_select('feedback_completedtmp', sprintf("id IN ($sql)", 'feedback_completedtmp'), $params);
        $DB->delete_records_select('feedback_value', sprintf("completed IN ($sql)", 'feedback_completed'), $params);
        $DB->delete_records_select('feedback_completed', sprintf("id IN ($sql)", 'feedback_completed'), $params);
    }
    ...
    public static function delete_data_for_user(approved_contextlist $contextlist) {
        ...
        $sql = "
            SELECT fc.id
             ...
             WHERE fc.userid = :userid
               AND cm.id $insql";
        $params = array_merge($inparams, ['userid' => $userid, 'feedback' => 'feedback']);
        $DB->delete_records_select('feedback_valuetmp', sprintf("completed IN ($sql)", 'feedback_completedtmp'), $params);
        $DB->delete_records_select('feedback_completedtmp', sprintf("id IN ($sql)", 'feedback_completedtmp'), $params);
        $DB->delete_records_select('feedback_value', sprintf("completed IN ($sql)", 'feedback_completed'), $params);
        $DB->delete_records_select('feedback_completed', sprintf("id IN ($sql)", 'feedback_completed'), $params);
    ...
    protected static function prepare_export_query(array $contextids, $userid) {
        ...
        $ctxfields = context_helper::get_preload_record_columns_sql('q');
        $sql = "
            SELECT q.*
