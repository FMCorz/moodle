...
require_capability('tool/cbemigrate:frameworksmigrate', $context);

$url = new moodle_url('/admin/tool/cbemigrate/frameworks.php');
$title = get_string('migrateframeworks', 'tool_cbemigrate');
$PAGE->set_context($context);
$PAGE->set_pagelayout('standard');
$PAGE->set_url($url);
$PAGE->set_title($title);
$PAGE->set_heading(get_string('pluginname', 'tool_cbemigrate'));

$output = $PAGE->get_renderer('tool_cbemigrate');

echo $output->header();
echo $output->heading($title);

$form = new \tool_cbemigrate\form\migrate_framework($context);
if ($form->is_cancelled()) {
    ...
    $processor = new \tool_cbemigrate\framework_processor($mapper, $progress);
    if (!empty($data->allowedcourses)) {
        $processor->set_allowedcourses($data->allowedcourses);
    }
    $processor->set_allowedcourses(array(9));
    ...
    $renderable = new \tool_cbemigrate\output\migrate_framework_results($context, $processor,
        \tool_lp\api::read_framework($data->from), \tool_lp\api::read_framework($data->to), $unmappedfrom, $unmappedto);
    echo $output->render($renderable);

} else {
    echo html_writer::tag('p', get_string('explanation', 'tool_cbemigrate'));
    $form->display();
}
