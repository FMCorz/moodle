...
define(['jquery',
    ...
    var confirmMove = function() {
        ...
        if (typeof targetComp.path && targetComp.path.indexOf('/' + sourceComp.id + '/') >= 0) {
            confirmMessage = 'movecompetencytochildofselfwillresetrules';
        }

        // Checking if the parent source, or the destination have rules.
        if ((treeModel.hasRule(targetComp.id) || treeModel.hasRule(sourceComp.parentid))) {
            str.get_strings([
                { key: 'confirm', component: 'moodle' },
