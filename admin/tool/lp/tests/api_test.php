...
class tool_lp_api_testcase extends advanced_testcase {
    ...
    public function test_add_evidence_complete() {
        ...
        // Trying to pass a grade should throw a debugging message.
        $evidence = api::add_evidence($u1->id, $c2->get_id(), $u1ctx->id, \tool_lp\evidence::ACTION_COMPLETE, 'invaliddata',
            'error', null, false, null, 1);
        $this->assertDebuggingCalled('A grade should not be set when completing a competency.');
        $evidence->read();
        $this->assertEquals(4, $evidence->get_grade());    // Still, the evidence recorded the default.
    }
