...
class tool_lp_persistent_testcase extends advanced_testcase {
    ...
    public function test_can_manage() {
        ...
        $this->setUser($u1);
        $this->assertTrue(\tool_lp\plan::can_manage($u1->id));
        $this->assertTrue(\tool_lp\plan::can_manage($u2->id));
        $this->assertTrue(\tool_lp\plan::can_manage($u3->id));

        $this->setUser($u2);
        $this->assertFalse(\tool_lp\plan::can_manage($u1->id));
        $this->assertTrue(\tool_lp\plan::can_manage($u2->id));
        $this->assertFalse(\tool_lp\plan::can_manage($u3->id));
    ...
    public function test_can_manage_draft() {
        $this->resetAfterTest(true);
        ...
        $this->setUser($u1);
        $this->assertTrue(\tool_lp\plan::can_manage_draft($u1->id));
        $this->assertTrue(\tool_lp\plan::can_manage_draft($u2->id));
        $this->assertTrue(\tool_lp\plan::can_manage_draft($u3->id));
        $this->assertTrue(\tool_lp\plan::can_manage_draft($u4->id));
        $this->assertTrue(\tool_lp\plan::can_manage_draft($u5->id));

        $this->setUser($u2);
        $this->assertFalse(\tool_lp\plan::can_manage_draft($u1->id));
        $this->assertTrue(\tool_lp\plan::can_manage_draft($u2->id));
        $this->assertFalse(\tool_lp\plan::can_manage_draft($u3->id));
        $this->assertFalse(\tool_lp\plan::can_manage_draft($u4->id));
        $this->assertFalse(\tool_lp\plan::can_manage_draft($u5->id));
        ...
        $this->setUser($u5);
        $this->assertFalse(\tool_lp\plan::can_manage_draft($u1->id));
        $this->assertFalse(\tool_lp\plan::can_manage_draft($u2->id));
        $this->assertFalse(\tool_lp\plan::can_manage_draft($u3->id));
        $this->assertFalse(\tool_lp\plan::can_manage_draft($u4->id));
        $this->assertTrue(\tool_lp\plan::can_manage_draft($u5->id));
    }
    ...
    public function test_can_read() {
        ...
        $this->setUser($u1);
        $this->assertTrue(\tool_lp\plan::can_read($u1->id));
        $this->assertTrue(\tool_lp\plan::can_read($u2->id));
        $this->assertTrue(\tool_lp\plan::can_read($u3->id));

        $this->setUser($u2);
        $this->assertFalse(\tool_lp\plan::can_read($u1->id));
        $this->assertTrue(\tool_lp\plan::can_read($u2->id));
        $this->assertFalse(\tool_lp\plan::can_read($u3->id));

        $this->setUser($u3);
        $this->assertFalse(\tool_lp\plan::can_read($u1->id));
        $this->assertTrue(\tool_lp\plan::can_read($u2->id));
        $this->assertTrue(\tool_lp\plan::can_read($u3->id));    // Due to the default capability.
    }

    public function test_can_read_draft() {
        $this->resetAfterTest(true);
        ...
        $this->setUser($u1);
        $this->assertTrue(\tool_lp\plan::can_read_draft($u1->id));
        $this->assertTrue(\tool_lp\plan::can_read_draft($u2->id));
        $this->assertTrue(\tool_lp\plan::can_read_draft($u3->id));
        $this->assertTrue(\tool_lp\plan::can_read_draft($u4->id));
        $this->assertTrue(\tool_lp\plan::can_read_draft($u5->id));

        $this->setUser($u2);
        $this->assertFalse(\tool_lp\plan::can_read_draft($u1->id));
        $this->assertTrue(\tool_lp\plan::can_read_draft($u2->id));
        $this->assertFalse(\tool_lp\plan::can_read_draft($u3->id));
        $this->assertFalse(\tool_lp\plan::can_read_draft($u4->id));
        $this->assertFalse(\tool_lp\plan::can_read_draft($u5->id));
        ...
        $this->setUser($u5);
        $this->assertFalse(\tool_lp\plan::can_read_draft($u1->id));
        $this->assertFalse(\tool_lp\plan::can_read_draft($u2->id));
        $this->assertFalse(\tool_lp\plan::can_read_draft($u3->id));
        $this->assertFalse(\tool_lp\plan::can_read_draft($u4->id));
        $this->assertTrue(\tool_lp\plan::can_read_draft($u5->id));
    }
